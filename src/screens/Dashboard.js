  ...
  const openwebapp = item => {
    if (item.title === 'Notification') {
      ...
      navigation.navigate('Webpanel', {
        ...
        url: `https://app.reliably.me/a/portfolio/?id=${userInfo._id}${'&isLogin=true'}`,
      });
      // Linking.openURL(`${CommunityPortfolio}?id=${userInfo._id}`);
    } else if (item.title === 'Action') {
      navigation.navigate('Webpanel', {
        name: 'Action',
        url: `https://app.reliably.me/a/portfolioactions/?id=${userInfo._id}${'&isLogin=true'}`,
      });
