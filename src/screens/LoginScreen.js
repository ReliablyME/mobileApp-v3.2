...
const LoginScreen = ({navigation}) => {
  ...
  const checkLogin = () => {
    if (!mobileNumber) {
      ...
      setmodalValue(true);
      setmessage('Verification code has been sent succusfully!');
      checkUserExist();
    }
  ...
  const checkUserExist = async () => {
    ...
    if (response.success) {
      //console.log(response["data"]["results"]["data"], 'check member login ');
  ...
  const sendOtp = async (status, usedetails) => {
    var response = await postApisWithoutToken(`/api/v2.0/getOTP`, {
    ...
    if (response?.data?.success) {
      ...
      navigation.navigate('OtpScreen', {
        ...
        userID: usedetails.data.results.data[0]._id,
      });
      setmodalValue(false);
    } else {
      console.log(response.data);
      setmodalValue(true);
      setmessage("Create an account on the ReliablyME website");
    }
  };
