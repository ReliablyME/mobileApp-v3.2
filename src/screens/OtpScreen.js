...
import {postApisWithoutToken} from '../apis';
...
const OtpScreen = ({navigation, route}) => {
  ...
  const completeLogin = async () => {
    ...
    if (!code) {
      ...
      var response = await postApisWithoutToken('/api/v2.0/verifyOTP', {
        otp: code,
        secret: issecret !== '' ? issecret : route.params?.secret,
      });

      console.log(response, 'submit verification code');

      if (response?.data?.success) {
        if (route.params.isUserExist === 1) {
          await AsyncStorage.setItem('userid', route.params.userID?.toString());
          navigation.navigate('Dashboard', {from: 'Otp'});
          await AsyncStorage.setItem('isLoggedIn', "true");
          await AsyncStorage.setItem('mobile', route.params.mobile?.toString());
        }
      } else {
        setmodalValue(true);
        setmessage(response?.data.data.errors[0]);
